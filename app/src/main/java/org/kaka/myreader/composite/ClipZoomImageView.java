...

{
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (isCanDrag) {
                    if (getDrawable() != null) {
                        ...
                        // 如果高度小雨屏幕高度，则禁止上下移动
                        if (rectF.height() <= getHeight() - verticalPadding * 2) {
                            dy = 0;
                        }
    ...
    public void onGlobalLayout() {
        if (once) {
            ...
            if (d == null)
                return;
            // 垂直方向的边距
            verticalPadding = (getHeight() - (getWidth() - 2 * horizontalPadding)) / 2;
            ...
            float scale = 1.0f;
            if (dw < getWidth() - horizontalPadding * 2
                    && dh > getHeight() - verticalPadding * 2) {
                scale = (getWidth() * 1.0f - horizontalPadding * 2) / dw;
            }

            if (dh < getHeight() - verticalPadding * 2
                    && dw > getWidth() - horizontalPadding * 2) {
                scale = (getHeight() * 1.0f - verticalPadding * 2) / dh;
            }

            if (dw < getWidth() - horizontalPadding * 2
                    && dh < getHeight() - verticalPadding * 2) {
                float scaleW = (getWidth() * 1.0f - horizontalPadding * 2)
                        / dw;
                float scaleH = (getHeight() * 1.0f - verticalPadding * 2) / dh;
    ...
    public Bitmap clip() {
        ...
        float scaleWidth = ((float) 300) / getWidth();
        float scaleHeight = scaleWidth;
        Matrix matrix = new Matrix();
        matrix.postScale(scaleWidth, scaleHeight);

        return Bitmap.createBitmap(bitmap, horizontalPadding,
                verticalPadding, getWidth() - 2 * horizontalPadding,
                getWidth() - 2 * horizontalPadding, matrix, true);
    }
    ...
    private void checkBorder() {
        ...
        if (rect.height() + 0.01 >= height - 2 * verticalPadding) {
